...
async function checkYahoo(ticker) {
  try {
    ...
    if (d?.chart?.error) {
      return { ok: false, reason: d.chart.error.description || d.chart.error.code };
    }
    const price = d?.chart?.result?.[0]?.meta?.regularMarketPrice;
    const longName = d?.chart?.result?.[0]?.meta?.longName || d?.chart?.result?.[0]?.meta?.shortName;
    if (price == null) return { ok: false, reason: 'No price in response' };
...
// GET /api/cron/ticker-audit — checks every OPEN position against Yahoo
// Finance right now (the source the cron jobs actually use), and reports
// which ones fail to resolve. Read-only unless ?notify=1 is passed.
export default async function handler(req, res) {
  ...
  try {
    ...
    for (const t of open) {
      ...
      const check = await checkYahoo(ticker);
      results.push({
        sym: t.sym,
        ticker,
