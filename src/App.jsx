  ...
  return (
    <div style={{position:'fixed',inset:0,zIndex:9000,background:'rgba(0,0,0,.75)',display:'flex',alignItems:'center',justifyContent:'center',padding:24}} onClick={e=>{if(e.target===e.currentTarget)onClose();}}>
      <div style={{width:'100%',maxWidth:500,background:'var(--bg2)',border:'1px solid var(--border)',borderRadius:20,padding:28,boxShadow:'0 32px 80px rgba(0,0,0,.7)',maxHeight:'90vh',overflowY:'auto'}}>
        ...
        <div style={{fontSize:11,color:'var(--txt3)',fontFamily:"'DM Mono'",marginBottom:20,lineHeight:1.7}}>Announcements are fetched free directly from NSE India — no signup needed. Optionally enable a daily Telegram summary below. Mainboard IPO opening and closing alerts are also sent automatically at 9:30 AM IST.</div>
...
function Dashboard({trades,setPage,setView,openPrices,onFetchPrices,priceLoading,lastPriceFetch}){
  ...
  return <div>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:28,flexWrap:"wrap",gap:12}}>
      <div>
        <div style={{fontFamily:"'Syne'",fontSize:26,fontWeight:700,letterSpacing:"-.02em",color:"var(--txt1)"}}>Your trading overview</div>
        <div style={{color:"var(--txt3)",fontSize:13,marginTop:4}}>Performance, positions, and your next move · {new Date().toLocaleDateString("en-IN",{weekday:"short",day:"numeric",month:"short"})}</div>
      </div>
      <div style={{display:"flex",alignItems:"flex-start",gap:10,flexWrap:"wrap"}}><button onClick={()=>setPage("add")} style={{padding:"10px 18px",borderRadius:12,border:"none",background:"var(--accent)",color:"#111",fontSize:13,fontWeight:700,cursor:"pointer"}}>+ Log a trade</button>{openCount > 0 && (
        <div style={{display:"flex",flexDirection:"column",alignItems:"flex-end",gap:4}}>
          <button
            onClick={onFetchPrices}
            disabled={priceLoading}
    ...
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(240px,1fr))",gap:14,marginBottom:16}}>
      {[
        {label:"Realised P&L",val:INR(totalNet,2),sub:`from ${closed.length} closed trade${closed.length!==1?'s':''}`,color:totalNet>=0?"var(--accent)":"var(--red)",spark},
...
/* ─── Journal ───────────────────────────────────────────────────────────────── */
function Journal({trades,onEdit,onDelete,setView,openPrices,setPage}){
  ...
  return <div>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-end",marginBottom:22}}>
      ...
      <div style={{display:"flex",gap:8}}><button onClick={exportCSV} style={{padding:"8px 14px",background:"transparent",border:"1px solid var(--border2)",borderRadius:8,color:"var(--txt3)",fontSize:12,cursor:"pointer"}}>↓ Export</button><button onClick={()=>setPage("add")} style={{padding:"8px 14px",background:"var(--accent)",border:"none",borderRadius:8,color:"#111",fontSize:12,fontWeight:700,cursor:"pointer"}}>+ New trade</button></div>
    </div>
    <div style={{display:"flex",gap:10,marginBottom:16,flexWrap:"wrap"}}>
      ...
      {(fQ||fS||fStr||fPnl) && <button onClick={()=>{setFQ("");setFS("");setFStr("");setFPnl("");}} style={{padding:"0 12px",border:"1px solid var(--border2)",borderRadius:8,background:"transparent",color:"var(--txt3)",fontSize:11,cursor:"pointer"}}>Clear filters</button>}
    ...
    <div style={{...C,padding:0,overflow:"hidden"}}>
      {data.length===0
        ?<div style={{padding:"3rem",textAlign:"center",color:"var(--txt3)",fontSize:13}}><div style={{fontSize:28,marginBottom:10}}>📒</div>No trades match these filters.<br/><button onClick={()=>{setFQ("");setFS("");setFStr("");setFPnl("");}} style={{marginTop:12,background:"none",border:"none",color:"var(--accent)",cursor:"pointer"}}>Clear filters</button></div>
  ...
  return <div>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:22}}>
      <div>
        ...
        <div style={{color:"var(--txt3)",fontSize:12,marginTop:3}}>Start with the essentials. Add notes and screenshots when you are ready.</div>
      </div>
      <div style={{display:"flex",gap:10}}>
    ...
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(280px,1fr))",gap:14}}>
      ...
      <div style={C}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(220px,1fr))",gap:12,marginTop:14}}>
          ...
          <div><FL ch="Trade status"/><div style={{display:"flex",gap:6,marginTop:6}}>{[["open","Open position"],["closed","Closed trade"]].map(([value,label])=><button key={value} type="button" onClick={()=>setStatus(value)} style={{flex:1,padding:"10px 8px",borderRadius:8,border:`1px solid ${status===value?"var(--accent)":"var(--border2)"}`,background:status===value?"rgba(0,229,160,.12)":"transparent",color:status===value?"var(--accent)":"var(--txt3)",fontSize:11,cursor:"pointer"}}>{label}</button>)}</div></div>
