...
import { supabase, isSupabaseConfigured, canUseSupabaseAuth } from './supabase.js';
    ...
    <div style={{...C,padding:0,overflow:"hidden"}}>
      {data.length===0
        ...
        :<div className="table-scroll"><table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}>
          <thead><tr style={{borderBottom:"1px solid var(--border)"}}>{["Symbol","Status","Entry","Entry ₹","Exit ₹","Qty","Investment","Net P&L ↕","Return","Strategy","★",""].map(h=><th key={h} style={{textAlign:"left",padding:"11px 12px",color:"var(--txt4)",fontSize:10,fontFamily:"'DM Mono'",letterSpacing:".05em",fontWeight:400,whiteSpace:"nowrap"}}>{h}</th>)}</tr></thead>
          <tbody>{data.map(t=>{const {invest,net,pct}=pnl(t);return <tr key={t.id} style={{borderBottom:"1px solid var(--border)",cursor:"pointer"}} onMouseEnter={e=>[...e.currentTarget.cells].forEach(c=>c.style.background="var(--bg3)")} onMouseLeave={e=>[...e.currentTarget.cells].forEach(c=>c.style.background="transparent")} onClick={()=>setView(t)}>
...
async function sbFetchTrades(userId){
  if(!supabase) return null;
  const { data, error } = await supabase.from('trades').select('data').eq('user_id', userId).order('created_at');
  if(error){ console.warn('sbFetchTrades error:', error.message); return null; }
  return data.map(r => r.data);
}
async function sbSaveTrade(userId, trade){
  if(!supabase) return;
  await supabase.from('trades').upsert({ id:trade.id, user_id:userId, data:trade }, { onConflict:'id' });
}
async function sbDeleteTrade(userId, tradeId){
  if(!supabase) return;
...
async function sbFetchGoals(userId){
  if(!supabase) return null;
  const { data } = await supabase.from('goals').select('data').eq('user_id', userId).single();
  return data?.data || null;
}
async function sbSaveGoals(userId, goals){
  if(!supabase) return;
  await supabase.from('goals').upsert({ user_id:userId, data:goals }, { onConflict:'user_id' });
}
async function sbFetchJournal(userId){
  if(!supabase) return null;
  const { data } = await supabase.from('daily_journal').select('date,data').eq('user_id', userId);
  if(!data) return null;
  return Object.fromEntries(data.map(r => [r.date, r.data]));
}
async function sbSaveJournalEntry(userId, date, entry){
  if(!supabase) return;
  await supabase.from('daily_journal').upsert({ user_id:userId, date, data:entry }, { onConflict:'user_id,date' });
}
async function sbFetchNotes(userId){
  if(!supabase) return null;
  const { data } = await supabase.from('sticky_notes').select('data').eq('user_id', userId);
  return data?.map(r=>r.data) || null;
}
async function sbSaveNote(userId, note){
  if(!supabase) return;
  await supabase.from('sticky_notes').upsert({ id:note.id, user_id:userId, data:note }, { onConflict:'id' });
}
async function sbDeleteNote(userId, noteId){
  if(!supabase) return;
  await supabase.from('sticky_notes').delete().eq('id', noteId).eq('user_id', userId);
}
async function sbFetchWatchlist(userId){
  if(!supabase) return null;
  const { data } = await supabase.from('watchlist').select('data').eq('user_id', userId);
  return data?.map(r=>r.data) || null;
}
async function sbSaveWatchlistItem(userId, item){
  if(!supabase) return;
  await supabase.from('watchlist').upsert({ id:item.id, user_id:userId, data:item }, { onConflict:'id' });
}
async function sbDeleteWatchlistItem(userId, itemId){
  if(!supabase) return;
  await supabase.from('watchlist').delete().eq('id', itemId).eq('user_id', userId);
}
...
function LoginForm({ onLogin, onError, setLoading, loading, cloudSyncEnabled }){
  ...
  const submit = async e => {
    ...
    if(cloudSyncEnabled && supabase){
      ...
      const session = { username:res.username, name:res.name };
      sessionSave(session);
      onLogin(session);
    }
    setLoading(false);
    ...
    <div style={{minHeight:'100vh',display:'flex',alignItems:'center',justifyContent:'center',padding:24,background:'var(--bg0)'}}>
      ...
      <div style={{position:'relative',zIndex:1,width:'100%',maxWidth:440}}>
        ...
        <div style={{textAlign:'center',marginTop:18,fontSize:11,color:'var(--txt4)',fontFamily:"'DM Mono'"}}>
          {cloudSyncEnabled ? 'Data synced to cloud · Access from any device' : 'Data stored locally · Add Supabase for cloud sync'}
        </div>
      </div>
    </div>
  );
...
export default function App(){
  ...
  const loadTradesFromSupabase = useCallback(async (session) => {
    ...
    if(cloudSyncEnabled && session.id){
  ...
  const logout = async () => {
    if(cloudSyncEnabled) await supabase.auth.signOut();
  ...
  const saveUserTrades = async (updated, changedTrade=null, deletedId=null) => {
    ...
    if(cloudSyncEnabled && user?.id){
      setSyncStatus('syncing');
      try{
        if(deletedId) await sbDeleteTrade(user.id, deletedId);
        else if(changedTrade) await sbSaveTrade(user.id, changedTrade);
