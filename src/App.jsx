...
function LoginForm({ onLogin, onError, setLoading, loading }){
  ...
  return (
    <form onSubmit={submit}>
      <div style={AUTH_FIELD}>
        <label style={AUTH_LABEL}>{isSupabaseConfigured ? 'Email' : 'Username'}</label>
        <input ref={emailRef} type={isSupabaseConfigured?'email':'text'} placeholder={isSupabaseConfigured?'your@email.com':'Enter your username'} autoComplete={isSupabaseConfigured?'email':'username'} defaultValue=""/>
      </div>
      <div style={AUTH_FIELD}>
      ...
      {!isSupabaseConfigured && <div style={{marginTop:12,textAlign:'center',fontSize:11,color:'var(--txt4)',fontFamily:"'DM Mono'"}}>{userCount} account{userCount!==1?'s':''} · local only</div>}
    </form>
  );
}

/* ── Signup Form ─────────────────────────────────────────────────────────────── */
function SignupForm({ onSuccess, onError, setLoading, loading }){
  ...
  const submit = async e => {
    ...
    if(isSupabaseConfigured && !email){ onError('Email is required for cloud sync.'); return; }
    setLoading(true); onError('');
    if(isSupabaseConfigured && supabase){
  ...
  return (
    <form onSubmit={submit}>
      ...
      <div style={AUTH_FIELD}><label style={AUTH_LABEL}>Email {isSupabaseConfigured?'(required)':'(optional)'}</label><input ref={emailRef} type="email" placeholder="your@email.com" autoComplete="email" defaultValue=""/></div>
      <div style={AUTH_FIELD}>
    ...
    <div style={{minHeight:'100vh',display:'flex',alignItems:'center',justifyContent:'center',padding:24,background:'var(--bg0)'}}>
      ...
      <div style={{position:'relative',zIndex:1,width:'100%',maxWidth:440}}>
        <div style={{textAlign:'center',marginBottom:32}}>
          ...
          {isSupabaseConfigured && <div style={{marginTop:8,fontSize:11,color:'var(--accent)',fontFamily:"'DM Mono'",background:'rgba(0,229,160,.08)',display:'inline-block',padding:'3px 10px',borderRadius:20,border:'1px solid rgba(0,229,160,.2)'}}>☁ Cloud sync enabled</div>}
        </div>
        <div style={{background:'var(--bg2)',border:'1px solid var(--border)',borderRadius:24,padding:36,boxShadow:'0 40px 100px rgba(0,0,0,.5)'}}>
          ...
          {mode==='login'
            ? <LoginForm  onLogin={onLogin} onError={setError} setLoading={setLoading} loading={loading}/>
            : <SignupForm onSuccess={msg=>{setSuccess(msg);switchMode('login');}} onError={setError} setLoading={setLoading} loading={loading}/>
          }
        </div>
        <div style={{textAlign:'center',marginTop:18,fontSize:11,color:'var(--txt4)',fontFamily:"'DM Mono'"}}>
          {isSupabaseConfigured ? 'Data synced to cloud · Access from any device' : 'Data stored locally · Add Supabase for cloud sync'}
        </div>
      </div>
    </div>
  );
