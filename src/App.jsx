...
import {
  ...
  sbFetchTelegramChatId,
  sbSaveTelegramChatId,
} from './supabase.js';
...
function AlertSettings({ onClose, onSaved, userId }){
  ...
  // Load existing Telegram chat ID from cloud storage on open
  useEffect(()=>{
    if(!userId){ setTgLoading(false); return; }
    sbFetchTelegramChatId(userId)
      .then(chatId => { setTgChatId(chatId || ''); setTgLoading(false); })
      .catch(() => setTgLoading(false));
  }, [userId]);

  const save = async () => {
    if(userId){
      await sbSaveTelegramChatId(userId, tgChatId.trim() || null);
    }
    setSaved(true);
    ...
    <div style={{position:'fixed',inset:0,zIndex:9000,background:'rgba(0,0,0,.75)',display:'flex',alignItems:'center',justifyContent:'center',padding:24}} onClick={e=>{if(e.target===e.currentTarget)onClose();}}>
      <div style={{width:'100%',maxWidth:500,background:'var(--bg2)',border:'1px solid var(--border)',borderRadius:20,padding:28,boxShadow:'0 32px 80px rgba(0,0,0,.7)',maxHeight:'90vh',overflowY:'auto'}}>
        ...
        <div style={{marginBottom:16,padding:'14px 16px',background:'var(--bg4)',borderRadius:12,border:'1px solid var(--border)'}}>
          ...
          {!userId && <div style={{fontSize:10,color:'var(--amber)',fontFamily:"'DM Mono'",marginTop:6}}>⚠ Sign in required to save your Telegram chat ID.</div>}
        </div>
...
function Dashboard({trades,setPage,setView,openPrices,onFetchPrices,priceLoading,lastPriceFetch}){
  ...
  const openLiveStatus=open.length>0 && openLiveData.length===0 ? 'Tap ⚡ Fetch Live Prices above' : `${INR(openLiveNet,2)} on ${openLiveData.length} of ${open.length} symbols`;
  let cum=0;
  ...
  return <div>
      ...
      {openCount > 0 && (
        <div style={{display:"flex",flexDirection:"column",alignItems:"flex-end",gap:4}}>
          ...
          >
            ...
            {priceLoading?`Fetching ${openCount}…`:`Live Prices (${openCount})`}
          </button>
          {lastPriceFetch && (
            <span style={{fontSize:10,color:"var(--txt4)",fontFamily:"'DM Mono'"}}>
              Updated {lastPriceFetch.toLocaleTimeString("en-IN",{hour:"2-digit",minute:"2-digit"})}
            </span>
          )}
        </div>
      )}
    ...
    {/* ── Hero P&L card ── */}
    <div style={{...C,marginBottom:14,display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:16,padding:"22px 24px"}}>
      <div>
        <div style={{fontSize:10,color:"var(--txt4)",fontFamily:"'DM Mono'",letterSpacing:".08em",marginBottom:8}}>NET P&amp;L · ALL TIME</div>
        <div style={{fontFamily:"'Syne'",fontSize:36,fontWeight:700,color:totalNet>=0?"var(--accent)":"var(--red)",letterSpacing:"-.02em",lineHeight:1}}>{INR(totalNet,2)}</div>
        <div style={{fontSize:12,color:"var(--txt3)",marginTop:8}}>on {INR(totalInvest,0)} deployed across {closed.length} closed trade{closed.length!==1?"s":""}</div>
      </div>
      {spark.length>2 && <div style={{opacity:.7}}><Spark data={spark} color={totalNet>=0?"var(--accent)":"var(--red)"}/></div>}
    </div>

    {/* ── Quick stat strip: 2-up on mobile, 4-up on desktop ── */}
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(150px,1fr))",gap:10,marginBottom:18}}>
      {[
        {label:"Open Positions",val:open.length,sub:open.length?openLiveStatus:"None open",color:"var(--txt1)"},
        {label:"Win Rate",val:winRate.toFixed(0)+"%",sub:`${wins.length}W · ${losses.length}L`,color:winRate>=55?"var(--accent)":winRate>=40?"var(--amber)":"var(--red)"},
        ...
        {label:"Total Trades",val:closed.length,sub:`${open.length} open now`,color:"var(--txt1)"},
      ].map((k,i)=>(
        <div key={i} style={{...C,padding:"14px 16px"}}>
          <div style={{fontSize:9,color:"var(--txt4)",fontFamily:"'DM Mono'",letterSpacing:".06em",marginBottom:8}}>{k.label.toUpperCase()}</div>
          <div style={{fontFamily:"'Syne'",fontSize:20,fontWeight:700,color:k.color,letterSpacing:"-.01em"}}>{k.val}</div>
          <div style={{fontSize:10,color:"var(--txt4)",marginTop:5,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{k.sub}</div>
        </div>
      ))}
    </div>
      ...
      {TABS.map(t=>{
        ...
        return <button key={t.id} onClick={()=>setTab(t.id)}
          style={{
            ...
            fontSize:13,fontWeight:active?700:500,fontFamily:"'DM Mono'",
    ...
    {tab==="overview" && <div style={{display:"grid",gap:14}}>
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(300px,1fr))",gap:14}}>
            ...
            <div style={{display:"flex",alignItems:"center",gap:10}}>
              ...
              {open.length>0 && (
                <button onClick={()=>setShowOpenPanel(true)} title="Expand"
                  style={{display:"flex",alignItems:"center",gap:5,padding:"7px 12px",borderRadius:7,border:"1px solid var(--border2)",background:"transparent",color:"var(--txt3)",fontSize:11,fontFamily:"'DM Mono'",cursor:"pointer",minHeight:36}}
                >⤢ Expand</button>
              )}
            </div>
          </div>
            ...
            :<div style={{display:"grid",gap:8}}>
              {open.slice(0,5).map(t=>{
                ...
                return <div key={t.id} onClick={()=>setView(t)}
                  style={{padding:"13px 14px",borderRadius:10,background:colors.bg,border:`1px solid ${colors.border}`,cursor:"pointer",minHeight:44}}
                >
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:10}}>
                    <div style={{minWidth:0}}>
                      <div style={{display:"flex",alignItems:"center",gap:8}}>
                        <span style={{fontFamily:"'Syne'",fontWeight:700,fontSize:14}}>{t.sym}</span>
                        {live && <span style={{fontSize:11,fontFamily:"'DM Mono'",fontWeight:700,color:live.changePct>=0?"var(--accent)":"var(--red)"}}>
                    ...
                    <div style={{textAlign:"right",flexShrink:0}}>
                      {live ? <>
                        <div style={{fontSize:14,fontWeight:700,fontFamily:"'DM Mono'",color:colors.color}}>
                          {live.net>=0?"+":"−"}₹{Math.abs(live.net).toLocaleString("en-IN",{maximumFractionDigits:0})}
                        </div>
                        <div style={{fontSize:11,fontFamily:"'DM Mono'",color:colors.color,marginTop:2}}>
              ...
              {open.length>5 && (
                <button onClick={()=>setShowOpenPanel(true)}
                  style={{marginTop:2,padding:"11px",borderRadius:8,border:"1px dashed var(--border2)",background:"transparent",color:"var(--txt3)",fontSize:12,fontFamily:"'DM Mono'",cursor:"pointer",minHeight:44}}
        ...
        <div style={C}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
            <SL ch="Recent Trades"/>
            <button onClick={()=>setPage("journal")} style={{fontSize:12,color:"var(--txt3)",background:"none",border:"none",cursor:"pointer",fontFamily:"'DM Mono'",minHeight:36,padding:"6px 8px"}}>View all →</button>
          </div>
          <div style={{display:"grid",gap:8}}>
            {recentTrades.length===0 && <div style={{textAlign:"center",padding:"2rem 1rem",color:"var(--txt4)",fontSize:12,fontFamily:"'DM Mono'"}}>No trades yet</div>}
            {recentTrades.map(t=>{
              const {net,pct}=pnl(t);
              return <div key={t.id} onClick={()=>setView(t)}
                style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"11px 12px",borderRadius:9,background:"var(--bg3)",cursor:"pointer",minHeight:44,gap:10}}
              >
                <div style={{minWidth:0}}>
                  <div style={{fontFamily:"'Syne'",fontWeight:700,fontSize:13}}>{t.sym}</div>
                  <div style={{fontSize:10,color:"var(--txt4)",marginTop:2}}>₹{t.entryPrice?.toLocaleString("en-IN")} → {t.exitPrice?`₹${t.exitPrice?.toLocaleString("en-IN")}`:"open"}</div>
                </div>
                <div style={{textAlign:"right",flexShrink:0}}>
                  {t.status==="open"
                    ? <StBadge s="open"/>
                    : <div style={{fontFamily:"'DM Mono'",fontSize:13,fontWeight:700,color:net>=0?"var(--accent)":"var(--red)"}}>{(net>=0?"+":"−")+"₹"+Math.abs(net).toLocaleString("en-IN",{maximumFractionDigits:0})}</div>}
                  {t.status!=="open" && <div style={{fontSize:10,fontFamily:"'DM Mono'",color:pct>=0?"var(--accent)":"var(--red)",marginTop:2}}>{PCT(pct)}</div>}
                </div>
              </div>;
            })}
          </div>
    ...
    {/* ── STRATEGIES TAB ── */}
    {tab==="strategies" && <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(220px,1fr))",gap:14}}>
      {strategySummary.map(group=>(
        <div key={group.name} onClick={()=>setSelectedStrategy(group.name)} style={{...C,cursor:"pointer",borderColor:selectedStrategy===group.name?"var(--accent)":"var(--border)",boxShadow:selectedStrategy===group.name?"0 0 0 1px rgba(0,229,160,.25)":"none",minHeight:44}}>
          <div style={{fontSize:12,color:"var(--txt2)",fontFamily:"'DM Mono'",letterSpacing:".08em",marginBottom:8}}>{group.name.toUpperCase()}</div>
          <div style={{fontFamily:"'Syne'",fontSize:22,fontWeight:700,color:group.net>=0?"var(--accent)":"var(--red)"}}>{INR(group.net,2)}</div>
      ...
      <div onClick={()=>setSelectedStrategy("All")} style={{...C,cursor:"pointer",borderColor:selectedStrategy==="All"?"var(--accent)":"var(--border)",boxShadow:selectedStrategy==="All"?"0 0 0 1px rgba(0,229,160,.25)":"none",minHeight:44}}>
        <div style={{fontSize:12,color:"var(--txt2)",fontFamily:"'DM Mono'",letterSpacing:".08em",marginBottom:8}}>ALL STRATEGIES</div>
        <div style={{fontFamily:"'Syne'",fontSize:22,fontWeight:700,color:"var(--txt1)"}}>{INR(filteredTrades.reduce((a,t)=>a+pnl(t).net,0),2)}</div>
        <div style={{display:"flex",justifyContent:"space-between",marginTop:12,fontSize:11,color:"var(--txt3)"}}>
    ...
    {/* ── CHARTS TAB ── */}
    {tab==="charts" && <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(260px,1fr))",gap:14}}>
      <div style={C}><SL ch="Monthly P&L"/><div style={{marginTop:12}}><BarChart trades={trades}/></div></div>
      <div style={{...C,display:"flex",flexDirection:"column",alignItems:"center",gap:12}}>
        ...
        <div style={{display:"flex",gap:20}}>
          {[["var(--accent)",`${wins.length} Wins`],["var(--red)",`${losses.length} Losses`]].map(([c,l])=>(
            <div key={l} style={{display:"flex",alignItems:"center",gap:6,fontSize:12,color:"var(--txt3)"}}><div style={{width:8,height:8,borderRadius:"50%",background:c}}/>{l}</div>
          ))}
        </div>
      </div>
      <div style={C}>
        ...
        <div style={{marginTop:12,display:"flex",flexDirection:"column",gap:10}}>
          {topSec.map(([sec,cnt])=>{const p=(cnt/trades.length)*100;return <div key={sec}>
            <div style={{display:"flex",justifyContent:"space-between",fontSize:12,color:"var(--txt3)",marginBottom:4}}><span>{sec}</span><span style={{fontFamily:"'DM Mono'",color:"var(--accent)"}}>{p.toFixed(0)}%</span></div>
            <div style={{height:5,borderRadius:3,background:"var(--bg4)"}}><div style={{width:p+"%",height:"100%",borderRadius:3,background:"var(--accent)",opacity:.7}}/></div>
          </div>;})}
        </div>
      </div>
    </div>}
...
function Journal({trades,onEdit,onDelete,setView,openPrices,onFetchClosedPrices,closedPriceLoading,lastClosedPriceFetch}){
  ...
  const activeFilterCount=[fS,fStr,fPnl].filter(Boolean).length;
  ...
  const clearFilters=()=>{setFS("");setFStr("");setFPnl("");};
  ...
  return <div>
    ...
    <div style={{display:"flex",gap:8,marginBottom:10}}>
      <input value={fQ} onChange={e=>setFQ(e.target.value)} placeholder="🔍 Search symbol or company…" style={{flex:1,minHeight:44}}/>
      <button onClick={()=>setShowFilters(s=>!s)} style={{
        padding:"0 16px",borderRadius:8,minHeight:44,
        border:`1px solid ${activeFilterCount?"var(--accent)":"var(--border2)"}`,
        background:activeFilterCount?"rgba(0,229,160,.08)":"transparent",
        color:activeFilterCount?"var(--accent)":"var(--txt3)",fontSize:13,cursor:"pointer",
        fontFamily:"'DM Mono'",whiteSpace:"nowrap",display:"flex",alignItems:"center",gap:6,
      }}>
        ⚙ Filters{activeFilterCount>0?` (${activeFilterCount})`:""}
      </button>
    </div>

    {/* ── Status quick-pills: always visible, fastest filter ── */}
    <div style={{display:"flex",gap:6,marginBottom:showFilters?10:16,flexWrap:"wrap"}}>
      {[{v:"",l:"All"},{v:"open",l:"Open"},{v:"closed",l:"Closed"}].map(o=>{
        const active=fS===o.v;
        return <button key={o.l} onClick={()=>setFS(o.v)} style={{
          padding:"7px 14px",borderRadius:999,fontSize:12,cursor:"pointer",minHeight:36,
          border:`1px solid ${active?"var(--accent)":"var(--border2)"}`,
          background:active?"rgba(0,229,160,.1)":"transparent",
          color:active?"var(--accent)":"var(--txt3)",fontWeight:active?700:500,
        }}>{o.l}</button>;
      })}
    </div>
    ...
    {showFilters && (
      <div style={{...C,padding:14,marginBottom:16,display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(160px,1fr))",gap:10}}>
        <div>
          <FL ch="Strategy"/>
          <select value={fStr} onChange={e=>setFStr(e.target.value)} style={{marginTop:6,width:"100%"}}><option value="">All Strategies</option>{strats.map(s=><option key={s}>{s}</option>)}</select>
        </div>
        <div>
          <FL ch="Result"/>
          <select value={fPnl} onChange={e=>setFPnl(e.target.value)} style={{marginTop:6,width:"100%"}}><option value="">All P&L</option><option value="profit">Profit only ↑</option><option value="loss">Loss only ↓</option></select>
        </div>
        <div>
          <FL ch="Sort by"/>
          <select value={sort} onChange={e=>setSort(e.target.value)} style={{marginTop:6,width:"100%"}}><option value="date-desc">Date (Newest)</option><option value="date-asc">Date (Oldest)</option><option value="pnl-desc">P&L (Best)</option><option value="pnl-asc">P&L (Worst)</option></select>
        </div>
        {activeFilterCount>0 && (
          <div style={{display:"flex",alignItems:"flex-end"}}>
            <button onClick={clearFilters} style={{padding:"9px 14px",borderRadius:8,border:"1px solid var(--border2)",background:"transparent",color:"var(--txt3)",fontSize:12,cursor:"pointer",minHeight:38,width:"100%"}}>Clear filters</button>
          </div>
        )}
      </div>
    )}
    ...
    {/* ── Trade cards ── */}
    {data.length===0
      ?<div style={{...C,padding:"3rem",textAlign:"center",color:"var(--txt4)",fontSize:12,fontFamily:"'DM Mono'"}}>
          <div style={{fontSize:28,marginBottom:10,opacity:.4}}>📭</div>
          No trades found{(fQ||activeFilterCount>0)?" for this filter":""}
        </div>
      :<div style={{display:"grid",gap:10}}>
        {data.map(t=>{
          const {invest,net,pct}=pnl(t);
          const live = t.status==="open" ? (()=>{const price=openPrices?.[t.sym]; if(price==null) return null; const gross = t.dir === "BUY" ? (price - t.entryPrice) * t.qty : (t.entryPrice - price) * t.qty; return { price, net: gross - (t.brokerage || 0) };})() : null;
          const displayNet = t.status==="open" ? (live?.net ?? null) : net;
          const displayColor = displayNet==null ? "var(--txt3)" : displayNet>=0?"var(--accent)":"var(--red)";
          return <div key={t.id} style={{...C,padding:14,cursor:"pointer"}} onClick={()=>setView(t)}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:10}}>
              <div style={{minWidth:0,flex:1}}>
                <div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}>
                  <span style={{fontFamily:"'Syne'",fontWeight:700,fontSize:15}}>{t.sym}</span>
                  <StBadge s={t.status}/>
                  {t.strategy && <span style={{fontSize:10,color:"var(--txt4)",fontFamily:"'DM Mono'"}}>· {t.strategy}</span>}
                </div>
                <div style={{fontSize:11,color:"var(--txt4)",marginTop:3}}>{t.sector} · {t.entryDate||"—"}</div>
              </div>
              <div style={{textAlign:"right",flexShrink:0}} onClick={e=>e.stopPropagation()}>
                <div style={{display:"flex",gap:5}}>
                  <IBtn title="Edit" ch="✎" onClick={()=>onEdit(t)}/>
                  <IBtn title="Delete" ch="✕" red onClick={()=>{if(confirm("Delete this trade?"))onDelete(t.id);}}/>
                </div>
              </div>
            </div>

            <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(90px,1fr))",gap:10,marginTop:12,paddingTop:12,borderTop:"1px solid var(--border)"}}>
              <div>
                <div style={{fontSize:9,color:"var(--txt4)",fontFamily:"'DM Mono'",letterSpacing:".05em"}}>ENTRY → EXIT</div>
                <div style={{fontSize:12,color:"var(--txt2)",fontFamily:"'DM Mono'",marginTop:3}}>₹{t.entryPrice?.toLocaleString("en-IN")} → {t.exitPrice?`₹${t.exitPrice.toLocaleString("en-IN")}`:"—"}</div>
              </div>
              <div>
                <div style={{fontSize:9,color:"var(--txt4)",fontFamily:"'DM Mono'",letterSpacing:".05em"}}>QTY</div>
                <div style={{fontSize:12,color:"var(--txt2)",fontFamily:"'DM Mono'",marginTop:3}}>{t.qty?.toLocaleString("en-IN")}</div>
              </div>
              <div>
                <div style={{fontSize:9,color:"var(--txt4)",fontFamily:"'DM Mono'",letterSpacing:".05em"}}>INVESTED</div>
                <div style={{fontSize:12,color:"var(--txt2)",fontFamily:"'DM Mono'",marginTop:3}}>₹{invest.toLocaleString("en-IN",{maximumFractionDigits:0})}</div>
              </div>
              <div>
                <div style={{fontSize:9,color:"var(--txt4)",fontFamily:"'DM Mono'",letterSpacing:".05em"}}>{t.status==="open"?"LIVE P/L":"NET P&L"}</div>
                <div style={{fontSize:13,color:displayColor,fontFamily:"'DM Mono'",fontWeight:700,marginTop:3}}>
                  {displayNet==null?"—":(displayNet>=0?"+":"−")+"₹"+Math.abs(displayNet).toLocaleString("en-IN",{maximumFractionDigits:0})}
                </div>
              </div>
              <div>
                <div style={{fontSize:9,color:"var(--txt4)",fontFamily:"'DM Mono'",letterSpacing:".05em"}}>RETURN</div>
                <div style={{fontSize:12,fontFamily:"'DM Mono'",color:pct>=0?"var(--accent)":"var(--red)",marginTop:3}}>{t.status==="open"?"—":PCT(pct)}</div>
              </div>
              <div>
                <div style={{fontSize:9,color:"var(--txt4)",fontFamily:"'DM Mono'",letterSpacing:".05em"}}>RATING</div>
                <div style={{marginTop:3,color:"var(--amber)",letterSpacing:1,fontSize:12}}>{"★".repeat(t.rating||0)}<span style={{color:"var(--txt4)"}}>{"★".repeat(5-(t.rating||0))}</span></div>
              </div>
            </div>

            {/* ── Held Till Today — hypothetical P&L if this closed trade was still open ── */}
            {t.status==="closed" && (()=>{
              const price=openPrices?.[t.sym];
              if(price==null) return null;
              const heldGross = t.dir==="BUY" ? (price-t.entryPrice)*t.qty : (t.entryPrice-price)*t.qty;
              const heldNet = heldGross - (t.brokerage||0);
              const delta = heldNet - net; // positive = you'd be better off having held
              const better = delta > 0;
              const roughlyEqual = Math.abs(delta) < 1;
              return (
                <div style={{marginTop:10,padding:"10px 12px",borderRadius:9,background:roughlyEqual?"var(--bg3)":better?"rgba(255,179,64,.07)":"rgba(0,229,160,.07)",border:`1px solid ${roughlyEqual?"var(--border)":better?"rgba(255,179,64,.22)":"rgba(0,229,160,.22)"}`}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:6}}>
                    <div style={{fontSize:10,color:"var(--txt4)",fontFamily:"'DM Mono'",letterSpacing:".05em"}}>📅 HELD TILL TODAY (CMP ₹{price.toLocaleString("en-IN")})</div>
                    <div style={{fontSize:12,fontFamily:"'DM Mono'",fontWeight:700,color:heldNet>=0?"var(--accent)":"var(--red)"}}>
                      {heldNet>=0?"+":"−"}₹{Math.abs(heldNet).toLocaleString("en-IN",{maximumFractionDigits:0})}
                    </div>
                  </div>
                  <div style={{fontSize:11,color:roughlyEqual?"var(--txt3)":better?"var(--amber)":"var(--accent)",marginTop:4}}>
                    {roughlyEqual
                      ? "Roughly the same as your actual exit"
                      : better
                        ? `You left ₹${Math.abs(delta).toLocaleString("en-IN",{maximumFractionDigits:0})} on the table by exiting early`
                        : `Good call — exiting saved you ₹${Math.abs(delta).toLocaleString("en-IN",{maximumFractionDigits:0})} vs holding`}
                  </div>
                </div>
              );
            })()}
...
function AddTrade({initial,onSave,onCancel}){
  ...
  // Advanced sections start open when editing an existing trade that already has that data,
  // so nothing is hidden/lost — but stay collapsed by default for a quick new-trade entry.
  const [showRisk,setShowRisk]=useState(!!(initial?.strategy||initial?.sl||initial?.target||initial?.rating||initial?.emotions?.length));
  const [showNotes,setShowNotes]=useState(!!(initial?.entryReason||initial?.exitReason||initial?.lessons||initial?.tags));
  const [showShots,setShowShots]=useState(!!(initial?.screenshots?.length));
  ...
  const SectionToggle=({open,setOpen,title,subtitle,filledCount})=>(
    <div onClick={()=>setOpen(o=>!o)} style={{...C,cursor:"pointer",display:"flex",justifyContent:"space-between",alignItems:"center",padding:"16px 20px",minHeight:44}}>
      <div>
        <div style={{fontFamily:"'Syne'",fontWeight:700,fontSize:14,display:"flex",alignItems:"center",gap:8}}>
          {title}
          {filledCount>0 && <span style={{fontSize:10,padding:"2px 8px",borderRadius:999,background:"rgba(0,229,160,.12)",color:"var(--accent)",fontFamily:"'DM Mono'"}}>{filledCount} added</span>}
        </div>
        <div style={{fontSize:11,color:"var(--txt4)",marginTop:3}}>{subtitle}</div>
      </div>
      <span style={{fontSize:16,color:"var(--txt3)",transform:open?"rotate(180deg)":"none",transition:"transform .2s"}}>⌄</span>
    </div>
  );

  const riskFilledCount=[strat,tf,sl,tgt,rating>0,emos.length>0].filter(Boolean).length;
  const notesFilledCount=[er,xr,less,tags].filter(Boolean).length;
  ...
  return <div>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:22,flexWrap:"wrap",gap:10}}>
      <div>
        ...
        <div style={{color:"var(--txt3)",fontSize:12,marginTop:3,fontFamily:"'DM Mono'"}}>{initial?"Update the details below":"Just the essentials below — add strategy, notes or charts if you want"}</div>
      </div>
      <div style={{display:"flex",gap:10}}>
    ...
    <div style={{display:"grid",gap:14}}>
      {/* Stock Search — always the first thing */}
      <div style={C}>
        <SL ch="1 · Which stock?"/>
      ...
      {/* Core trade details — always visible, this is all you need for a quick log */}
      <div style={C}>
        <SL ch="2 · Trade Details"/>
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(220px,1fr))",gap:12,marginTop:14}}>
          ...
          {status==="closed" && <div><FL ch="Exit Date"/><input type="date" value={xd} onChange={e=>setXD(e.target.value)} style={{marginTop:6}}/></div>}
          <div><FL ch="Entry Price ₹"/><input type="number" placeholder="0.00" value={ep} onChange={e=>setEP(e.target.value)} style={{marginTop:6}}/></div>
          {status==="closed" && <div><FL ch="Exit Price ₹"/><input type="number" placeholder="0.00" value={xp} onChange={e=>setXP(e.target.value)} style={{marginTop:6}}/></div>}
      ...
      {/* ── Optional sections, collapsed by default to cut clutter ── */}
      <SectionToggle open={showRisk} setOpen={setShowRisk} title="3 · Strategy & Risk" subtitle="Optional — strategy tag, stop loss, target, rating, mindset" filledCount={riskFilledCount}/>
      {showRisk && <div style={C}>
        ...
        <div style={{marginTop:14}}>
          <FL ch="Trade Rating"/>
          <div style={{display:"flex",gap:6,marginTop:8}}>{[1,2,3,4,5].map(n=><span key={n} onClick={()=>setRating(n)} style={{fontSize:24,cursor:"pointer",color:n<=rating?"var(--amber)":"#222",padding:"4px"}}>★</span>)}</div>
        </div>
        <div style={{marginTop:14}}>
          <FL ch="Emotion / Mindset"/>
          <div style={{display:"flex",flexWrap:"wrap",gap:6,marginTop:8}}>{EMOS.map(e=><span key={e} onClick={()=>setEmos(p=>p.includes(e)?p.filter(x=>x!==e):[...p,e])} style={{padding:"6px 12px",borderRadius:20,fontSize:11,cursor:"pointer",border:`1px solid ${emos.includes(e)?"var(--accent)":"var(--border2)"}`,background:emos.includes(e)?"#00E5A012":"transparent",color:emos.includes(e)?"var(--accent)":"var(--txt3)",fontFamily:"'DM Mono'",transition:"all .15s",minHeight:32,display:"inline-flex",alignItems:"center"}}>{e}</span>)}</div>
        </div>
      </div>}

      <SectionToggle open={showNotes} setOpen={setShowNotes} title="4 · Trade Notes" subtitle="Optional — entry/exit reasoning, lessons, tags" filledCount={notesFilledCount}/>
      {showNotes && <div style={C}>
      ...
      <SectionToggle open={showShots} setOpen={setShowShots} title="5 · Chart Screenshots" subtitle="Optional" filledCount={shots.length}/>
      {showShots && <div style={C}>
        <div onClick={()=>fileRef.current.click()} style={{border:"1.5px dashed var(--border2)",borderRadius:10,padding:"1.5rem",textAlign:"center",cursor:"pointer",background:"var(--bg2)",transition:"border .15s",minHeight:44}} onMouseEnter={e=>e.currentTarget.style.borderColor="#3a3a50"} onMouseLeave={e=>e.currentTarget.style.borderColor="var(--border2)"}>
        ...
        {shots.length>0&&<div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(140px,1fr))",gap:10,marginTop:12}}>
          {shots.map((s,i)=><div key={i} style={{position:"relative",aspectRatio:"16/9",borderRadius:8,overflow:"hidden",border:"1px solid var(--border2)"}}>
            ...
            <button onClick={()=>setShots(p=>p.filter((_,j)=>j!==i))} style={{position:"absolute",top:4,right:4,background:"rgba(0,0,0,.75)",border:"none",borderRadius:"50%",width:22,height:22,color:"#fff",cursor:"pointer",fontSize:10}}>✕</button>
          </div>)}
        </div>}
      </div>}
    </div>
...
/* ─── Trade View Modal ──────────────────────────────────────────────────────── */
function Modal({t,openPrices,onFetchClosedPrices,closedPriceLoading,onClose}){
  ...
  const heldTillToday = t.status==="closed" && openPrices?.[t.sym]!=null ? (()=>{
    const price=openPrices[t.sym];
    const g=t.dir==="BUY"?(price-t.entryPrice)*t.qty:(t.entryPrice-price)*t.qty;
    const n=g-(t.brokerage||0);
    return {price,net:n,delta:n-net};
  })():null;
  ...
  return (
    <div className="modal-overlay" onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div className="modal-box">
        ...
        {/* ── Held Till Today — closed trades only ── */}
        {t.status==="closed" && !heldTillToday && (
          <button
            onClick={()=>onFetchClosedPrices([{sym:t.sym,ticker:t.ticker||(t.sym+'.NS')}])}
            disabled={closedPriceLoading}
            style={{width:"100%",marginBottom:18,padding:"12px 14px",borderRadius:12,border:"1px dashed var(--border2)",background:"var(--bg4)",color:closedPriceLoading?"var(--txt4)":"var(--accent)",fontSize:12,fontFamily:"'DM Mono'",cursor:closedPriceLoading?"default":"pointer",minHeight:44}}
          >{closedPriceLoading?"⟳ Checking current price…":"📅 See what today's price implies"}</button>
        )}
        {heldTillToday && (()=>{
          const better = heldTillToday.delta > 0;
          const roughlyEqual = Math.abs(heldTillToday.delta) < 1;
          return (
            <div style={{background:roughlyEqual?"var(--bg4)":better?"rgba(255,179,64,.06)":"rgba(0,229,160,.06)",border:`1px solid ${roughlyEqual?"var(--border)":better?"rgba(255,179,64,.18)":"rgba(0,229,160,.18)"}`,borderRadius:14,padding:"14px 16px",marginBottom:18}}>
              <div style={{fontSize:10,color:roughlyEqual?"var(--txt3)":better?"var(--amber)":"var(--accent)",fontFamily:"'DM Mono'",letterSpacing:".06em",marginBottom:10}}>📅 IF YOU HELD TILL TODAY</div>
              <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8,marginBottom:10}}>
                <Pill label="CMP" value={"₹"+heldTillToday.price.toLocaleString("en-IN",{minimumFractionDigits:2})} color="var(--txt1)"/>
                <Pill label="Hypothetical P&L" value={(heldTillToday.net>=0?"+":"−")+INR(Math.abs(heldTillToday.net),0)} color={heldTillToday.net>=0?"var(--accent)":"var(--red)"}/>
                <Pill label="vs Actual Exit" value={(heldTillToday.delta>=0?"+":"−")+INR(Math.abs(heldTillToday.delta),0)} color={roughlyEqual?"var(--txt2)":better?"var(--amber)":"var(--accent)"}/>
              </div>
              <div style={{fontSize:12,color:roughlyEqual?"var(--txt3)":better?"var(--amber)":"var(--accent)"}}>
                {roughlyEqual
                  ? "Roughly the same as your actual exit."
                  : better
                    ? `You left ₹${Math.abs(heldTillToday.delta).toLocaleString("en-IN",{maximumFractionDigits:0})} on the table by exiting early.`
                    : `Good call — exiting saved you ₹${Math.abs(heldTillToday.delta).toLocaleString("en-IN",{maximumFractionDigits:0})} vs holding.`}
              </div>
            </div>
          );
        })()}
...
export default function App(){
  ...
  const fetchClosedPrices = useCallback(async (symList) => {
    ...
    const unique = (symList || trades.filter(t => t.status === "closed" && t.sym))
      .map(t => ({ sym: t.sym, ticker: t.ticker || (t.sym + '.NS') }))
      .filter(t => seen.has(t.sym) ? false : seen.add(t.sym));
    if (unique.length === 0) return;
    setClosedPriceLoading(true);
    const fetched = await fetchBulkLTP(unique);
      ...
      <>
        <div className="app-shell">
          ...
          <div className="app-content">
            ...
            {page==="goals"     && <GoalsPage trades={trades} username={user?.username} userId={user?.id}/>}
            {page==="announcements" && <AnnouncementsPage username={user?.username} userId={user?.id}/>}
            {page==="ipo"           && <IPOPage username={user?.username} userId={user?.id}/>}
        ...
        {viewing && <Modal t={viewing} openPrices={openPrices} onFetchClosedPrices={fetchClosedPrices} closedPriceLoading={closedPriceLoading} onClose={()=>setViewing(null)}/>}
